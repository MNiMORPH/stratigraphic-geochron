...
- The **joint posterior** (panel D) answers *when did this one event happen,
  given that every date constrains it?* It is always sharper than its inputs.
...
| `age_brackets.png` / `.pdf` | the output. Regenerated by the script; the PDF is byte-reproducible. |
| `requirements.txt` | the five packages needed. |
| `LICENSE` | GPL v3. |
| `CITATION.cff` | how to cite this, plus the sources it depends on. |
